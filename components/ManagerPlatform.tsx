...
export const ManagerPlatform: React.FC<ManagerPlatformProps> = ({ onBack }) => {
    ...
    return (
        <div className="absolute inset-0 bg-gray-900 flex flex-col text-white overflow-hidden font-sans transition-colors duration-500">
            {/* Header & Tabs */}
            <div className="bg-gray-800 border-b border-gray-700 flex-shrink-0 shadow-xl z-10 transition-colors">
                <div className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-4">
                        <button 
                            onClick={onBack} 
                            className="bg-gray-700 p-2 rounded-full hover:bg-gray-600 border border-gray-600 transition-all active:scale-95 text-gray-300"
                        >
                            <ArrowLeft size={20} />
                        </button>
                    ...
                    <button 
                        onClick={() => loadData()} 
                        disabled={isLoading}
                        className="p-2 hover:bg-gray-700 rounded-full text-[#4fc3f7] transition-colors disabled:opacity-50"
                    >
                ...
                <div className="flex px-4 gap-6 overflow-x-auto no-scrollbar">
                    <button 
                        ...
                        className={`pb-3 px-2 text-sm font-bold border-b-2 transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === 'open_orders' ? 'border-[#ffeb3b] text-yellow-400' : 'border-transparent text-gray-400 hover:text-white'}`}
                    >
                        <List size={18} className={activeTab === 'open_orders' ? 'text-yellow-400' : ''} />
                        Pedidos Abertos
                    </button>
                    <button 
                        onClick={() => setActiveTab('finished_orders')}
                        className={`pb-3 px-2 text-sm font-bold border-b-2 transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === 'finished_orders' ? 'border-[#00e676] text-[#00e676]' : 'border-transparent text-gray-400 hover:text-white'}`}
                    >
                    ...
                    <div className="space-y-4 max-w-4xl mx-auto pb-10">
                        {openOrders.length === 0 && !isLoading && (
                             <div className="text-center py-12 text-gray-500 flex flex-col items-center">
                                <div className="bg-gray-800 p-4 rounded-full mb-4 shadow">
                        ...
                        {openOrders.map(order => (
                            <div key={order.id} className="bg-gray-800 rounded-xl border border-gray-700 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-lg hover:border-[#4fc3f7] transition-colors">
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-gray-400">
                                        <span className="flex items-center gap-1 bg-gray-700 px-2 py-1 rounded border border-gray-600">
                                            <Clock size={12}/> {new Date(order.createdAt).toLocaleString()}
                                        </span>
                                        <span className="bg-[#0277bd]/20 text-[#0277bd] px-2 py-1 rounded font-bold border border-[#0277bd]/30">
                                            {order.items.length} ITENS
                                        </span>
                                    </div>
                                </div>
                                <div className="flex items-center justify-end sm:border-l border-gray-700 sm:pl-4">
                                    <button 
                                        onClick={() => handleDeleteOrder(order.id)} 
                                        className="text-red-400 hover:bg-red-900/30 p-2 px-3 rounded-lg transition-colors flex items-center gap-2 text-sm font-bold border border-red-900/50 hover:border-red-500"
                                    >
                    ...
                    <div className="space-y-4 max-w-4xl mx-auto pb-10">
                        ...
                        {completedOrders.map(order => (
                            <div 
                                ...
                                className="bg-gray-800/80 rounded-xl border border-gray-700 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm hover:bg-gray-800 transition-colors cursor-pointer group"
                            >
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2">
                                ...
                                <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                    <button 
                                        ...
                                        className="p-2 text-gray-500 hover:text-[#4fc3f7] hover:bg-gray-700 rounded-full transition-colors"
            ...
            {selectedOrder && (
                <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl transition-colors">
                        <div className="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50 rounded-t-2xl">
                            <div>
                                <div className="text-xs text-gray-400 uppercase font-bold">Detalhes do Pedido</div>
                                <h2 className="text-xl font-bold text-white">{selectedOrder.name}</h2>
                            </div>
                            <button onClick={() => setSelectedOrder(null)} className="p-2 hover:bg-gray-700 rounded-full text-gray-400 transition-colors">
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-0">
                            <table className="w-full text-sm text-left">
                                <thead className="text-xs text-gray-400 bg-gray-900 uppercase font-bold sticky top-0">
                                    <tr>
                        ...
                        <div className="p-4 border-t border-gray-700 bg-gray-900/50 rounded-b-2xl flex justify-end">
                             <button 
                                ...
                                className="text-red-400 text-sm font-bold flex items-center gap-2 hover:bg-red-900/30 px-4 py-2 rounded-lg transition-colors border border-transparent hover:border-red-900/50"
                             >
