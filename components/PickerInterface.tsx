...
export const PickerInterface: React.FC<PickerInterfaceProps> = ({ onSwitchToManager }) => {
  ...
  useEffect(() => {
    ...
    // Auto-fetch Stock and Orders on load (only once)
    if (appStep === 'main') {
        refreshCloudData();
    }
  }, [appStep]);
  ...
  const refreshCloudData = async () => {
      ...
      try {
          ...
          // Only show alert if user explicitly clicked refresh, otherwise just set error state
          if (isSetupOpen) {
               setCloudError(msg);
          }
      } finally {
  ...
  // --- Handlers ---
  const selectCloudOrder = async (order: CloudOrder) => {
  ...
  const calculateShortages = (currentOrders: OrderItem[], currentStock: StockItem[]) => {
    ...
    currentOrders.forEach(order => {
        ...
        // Handle missing materials (0 availability)
  ...
  const generateRoute = () => {
    ...
    if(tasks.length > 0) {
        ...
        // No picking tasks? Alert but allow entry to summary
        alert("Atenção: Nenhum material da encomenda foi encontrado no stock com localização válida. Verifique o Resumo.");
  ...
  if (appStep === 'warehouse-select') return (
      <div className="w-screen h-screen bg-[#1a2332] flex items-center justify-center p-6 relative">
          <div className="max-w-md w-full bg-[#141923] border border-[#37474f] rounded-2xl shadow-2xl p-8 flex flex-col items-center gap-6">
               ...
               <div className="w-full pt-4 border-t border-[#37474f] flex justify-center">
                    <button onClick={onSwitchToManager} className="text-gray-500 hover:text-[#4fc3f7] text-sm flex items-center gap-2 transition-colors">
                        <Database size={14} /> Plataforma de Gestão
                    </button>
               </div>
          </div>
      </div>
  ...
  if (appStep === 'password') return (
      <div className="w-screen h-screen bg-[#1a2332] flex items-center justify-center p-6 relative">
           <button onClick={() => setAppStep('warehouse-select')} className="absolute top-6 left-6 text-gray-400 hover:text-white flex items-center gap-2"><ArrowLeft /> Voltar</button>
        ...
        <div className="absolute inset-0 z-50 bg-[#1a2332] flex flex-col animate-fade-in">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
             ...
             <div className="bg-[#141923] p-4 rounded-lg border border-[#37474f]">
                ...
                {/* Error Banner */}
                {cloudError && (
                    <div className="bg-red-900/50 border border-red-500/50 p-3 rounded mb-4 flex items-start gap-2">
                        <AlertTriangle className="text-red-400 shrink-0 mt-0.5" size={16} />
                        <div>
                            <div className="text-sm text-red-200 font-bold">Erro de Sincronização</div>
                            <div className="text-xs text-red-300">{cloudError}</div>
                        </div>
                    </div>
                )}
                ...
                <div className="space-y-2 max-h-[60vh] overflow-y-auto min-h-[100px]">
                    {cloudOrders.length === 0 ? (
                        <div className="text-center text-gray-500 py-8 flex flex-col items-center">
                            ...
                            <p>{cloudError ? "Erro ao carregar pedidos" : "Sem pedidos pendentes"}</p>
                        </div>
                    ) : (
                        cloudOrders.map(order => (
                            <button 
                                ...
                                className={`w-full text-left p-4 rounded-lg border transition-all flex justify-between items-center ${activeOrderName === order.name ? 'bg-[#263238] border-[#00e676]' : 'bg-[#1e2736] border-[#37474f] hover:border-[#4fc3f7] disabled:opacity-50 disabled:cursor-wait'}`}
                            >
                                <div>
                                ...
                                <div className="text-right">
                                    {isProcessingOrder && activeOrderName === order.name ? (
                                        <Loader2 className="animate-spin text-[#00e676]" />
                                    ) : (
