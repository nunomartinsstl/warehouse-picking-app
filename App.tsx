...
import { Settings, Search, CheckCircle, X, Download, Plus, Trash2, Edit3, FileSpreadsheet, List, Camera, ChevronRight, ScanLine, Warehouse, Lock, FileText, AlertCircle, RefreshCw, AlertTriangle, Layers, MapPin, ThumbsUp, RotateCcw, Cloud, CloudUpload, CloudDownload } from 'lucide-react';
import { Scene3D } from './components/Scene3D';
import { FileUpload } from './components/FileUpload';
import { parseExcel, processOrderFile, processStockFile } from './utils/excelParser';
import { saveStockToCloud, fetchStockFromCloud, saveOrderToCloud, fetchOpenOrdersFromCloud, markOrderComplete } from './utils/firebase';
...
const App: React.FC = () => {
  ...
  const [cloudOrders, setCloudOrders] = useState<{name: string, date: string, items: OrderItem[]}[]>([]);
  const [activeTab, setActiveTab] = useState<'local' | 'cloud'>('cloud'); // Default to Cloud
  ...
  const [pickListViewMode, setPickListViewMode] = useState<'summary' | 'detailed'>('detailed'); // Default to Detailed (Ver Rota)
  const [visibleFloor, setVisibleFloor] = useState<number | null>(null); // Null = All
  ...
  const scanLockRef = useRef(false); // Prevents multiple rapid scans
  ...
  // Update visible floor based on current task if in 'auto' mode? 
  useEffect(() => {
      if (focusedTaskIndex !== null && pickingTasks[focusedTaskIndex]) {
          // Optional: Auto switch floor logic removed to keep manual control preference
      }
  }, [focusedTaskIndex, pickingTasks]);
  ...
  const handleStockUpload = async (file: File) => {
    try {
      ...
      if (confirm("Deseja enviar este stock para a Cloud para que outros utilizadores o vejam?")) {
          setIsCloudSyncing(true);
          await saveStockToCloud(processed);
          alert("Stock atualizado na Cloud com sucesso!");
          setIsCloudSyncing(false);
      }
  ...
  const selectOrderFile = async (file: File) => {
    try {
      ...
      if (confirm("Deseja disponibilizar esta encomenda na Cloud?")) {
          setIsCloudSyncing(true);
          await saveOrderToCloud(name, processed);
          alert("Encomenda enviada para a Cloud!");
          setIsCloudSyncing(false);
          refreshCloudData();
      }
  ...
  const selectCloudOrder = (order: {name: string, date: string, items: OrderItem[]}) => {
      setActiveOrderName(order.name);
      setActiveOrderDate(order.date);
  ...
  const generateRoute = () => {
    ...
    if(tasks.length > 0) {
        ...
        setVisibleFloor(tasks[0].floorId); // Auto switch to start floor
    }
  ...
  const exportAndFinish = async () => {
      ...
      // Update Cloud Status
      if (activeOrderName && activeTab === 'cloud') {
          await markOrderComplete(activeOrderName);
          refreshCloudData();
      }
  ...
  const confirmAdHocItem = (sequence: number) => {
      setCompletedTasks(prev => prev.map(t => 
          t.sequence === sequence ? { ...t, requiresConfirmation: false } : t
      ));
  };
  ...
  const getValidationInfo = (task: PickingTask) => {
      ...
      // Calculate TOTAL Global Stock for this material
      const totalGlobalStock = stock
        .filter(s => s.material === task.material)
        .reduce((acc, curr) => acc + curr.qtyAvailable, 0);

      // Calculate Total Order Quantity for this material
      const orderTotal = orders
        .filter(o => o.material === task.material)
        .reduce((a, c) => a + c.qty, 0);

      // Insufficient stock is only true if GLOBAL stock is less than Order Total
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-gray-900 text-white font-sans">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 p-4 pointer-events-none">
        <div className="flex gap-2 pointer-events-auto mb-2">
          ...
          <button onClick={() => setIsHistoryOpen(true)} className="bg-[#263238] p-3 rounded-lg shadow-lg border border-[#37474f] text-white hover:bg-[#37474f] transition-colors"><FileSpreadsheet size={24} className="text-[#4fc3f7]" /></button>
      ...
      {isSetupOpen && (
        <div className="absolute inset-0 z-50 bg-[#1a2332] flex flex-col animate-fade-in">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
             ...
             <div className="bg-[#141923] p-4 rounded-lg border border-[#37474f] space-y-4">
                <div className="flex justify-between items-center">
                    <h3 className="text-sm font-bold text-[#ffeb3b] uppercase flex items-center gap-2"><FileSpreadsheet size={16} /> Selecionar Pedido</h3>
                    <div className="flex bg-[#263238] rounded p-1">
                        <button onClick={() => setActiveTab('cloud')} className={`px-3 py-1 rounded text-xs font-bold transition-colors ${activeTab === 'cloud' ? 'bg-[#0277bd] text-white' : 'text-gray-400'}`}>CLOUD</button>
                        <button onClick={() => setActiveTab('local')} className={`px-3 py-1 rounded text-xs font-bold transition-colors ${activeTab === 'local' ? 'bg-[#0277bd] text-white' : 'text-gray-400'}`}>LOCAL</button>
                    </div>
                </div>

                {activeTab === 'local' ? (
                    <>
                        <FileUpload label="Carregar Pasta (Local)" folderMode={true} onFilesSelect={handleOrdersFolderSelect} status={availableOrders.length > 0 ? 'loaded' : 'idle'} />
                    ...
                    <div className="mt-4 max-h-60 overflow-y-auto bg-[#1a2332] rounded border border-[#37474f] min-h-[150px]">
                        {isCloudSyncing ? (
                            <div className="flex flex-col items-center justify-center py-8 text-gray-400">
                                <RefreshCw className="animate-spin mb-2" /> A sincronizar...
                            </div>
                        ) : cloudOrders.length === 0 ? (
                            <div className="p-4 text-center text-gray-500 text-sm">Nenhuma encomenda disponível na Cloud</div>
                        ) : (
                            cloudOrders.map((order, i) => (
                                <div key={i} onClick={() => selectCloudOrder(order)} className={`p-3 border-b border-[#2d3a4b] cursor-pointer hover:bg-[#263238] ${activeOrderName === order.name ? 'bg-[#263238] border-l-4 border-l-[#00e676]' : ''}`}>
                                    <div className="flex justify-between items-center">
                                        <div className="font-bold text-white">{order.name}</div>
                                        <CloudDownload size={16} className="text-[#0277bd]" />
                                    </div>
                                    <div className="text-xs text-gray-500 mt-1">{order.date} • {order.items.length} itens</div>
                                </div>
                            ))
                        )}
                    </div>
             ...
             <div className="bg-[#141923] p-4 rounded-lg border border-[#37474f]">
                 <div className="flex justify-between items-center mb-3">
                     <h3 className="text-sm font-bold text-[#4fc3f7] uppercase flex items-center gap-2"><Layers size={16} /> Stock do Armazém</h3>
                     {cloudStockDate && <span className="text-[10px] text-gray-500 flex items-center gap-1"><Cloud size={10} /> Atualizado: {cloudStockDate}</span>}
                 </div>
                 
                 <div className="grid grid-cols-2 gap-2">
                     <div className="col-span-2">
                        <FileUpload label="Carregar & Atualizar Cloud" onFileSelect={handleStockUpload} status={stockStatus} />
                     </div>
                     <button onClick={refreshCloudData} className="col-span-2 py-3 bg-[#263238] text-gray-300 font-bold text-sm rounded hover:bg-[#37474f] flex items-center justify-center gap-2 border border-[#37474f]">
                        <RefreshCw size={16} className={isCloudSyncing ? "animate-spin" : ""} /> Forçar Sincronização Cloud
                     </button>
                 </div>
             </div>
          ...
          <div className="p-4 border-t border-[#37474f] bg-[#141923]">
            <button onClick={generateRoute} className="w-full bg-[#0277bd] text-white font-bold py-4 rounded-lg shadow-lg">INICIAR PICKING</button>
          </div>
        </div>
      )}
      ...
      {isHistoryOpen && (
          <div className="absolute inset-0 z-50 bg-[#1a2332] flex flex-col animate-fade-in">
             <div className="p-4 border-b border-[#37474f] flex justify-between items-center bg-[#141923]">
                <h2 className="text-xl font-bold text-[#ff9800]">Histórico de Sessões</h2>
                <button onClick={() => setIsHistoryOpen(false)}><X size={24} /></button>
             </div>
             <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {sessionHistory.length === 0 && <div className="text-center text-gray-400 mt-10">Sem histórico</div>}
                {sessionHistory.map((session, idx) => (
                    <div key={`${session.id}-${idx}`} className="bg-[#1e2736] rounded-lg border border-[#37474f] overflow-hidden">
                        ...
                        <div className="grid grid-cols-2 divide-x divide-[#37474f] border-t border-[#37474f]">
                            <button onClick={() => resumeSession(session)} className="p-3 flex justify-center items-center gap-2 hover:bg-[#37474f]"><Edit3 size={18} className="text-[#00e676]" /> Detalhes</button>
                            <button onClick={() => generateExcelFile(session.tasks, session.orderName)} className="p-3 flex justify-center items-center gap-2 hover:bg-[#37474f]"><Download size={18} className="text-[#4fc3f7]" /> Excel</button>
                        </div>
                    </div>
                ))}
             </div>
      ...
      {/* Main Task View (Bottom Card) */}
      {!isSetupOpen && !isHistoryOpen && !isCompletionOpen && !searchQuery && !pendingConfirmation && !isMaterialSelectOpen && pickingTasks.length > 0 && (
